...
const DeclarativeAgentManifestV1D7 = __importStar(require("./copilot/declarative-agent/DeclarativeAgentManifestV1D7"));
exports.DeclarativeAgentManifestV1D7 = DeclarativeAgentManifestV1D7;
const DeclarativeAgentManifestV1D8 = __importStar(require("./copilot/declarative-agent/DeclarativeAgentManifestV1D8"));
exports.DeclarativeAgentManifestV1D8 = DeclarativeAgentManifestV1D8;
...
const TeamsManifestV1D27 = __importStar(require("./teams/TeamsManifestV1D27"));
exports.TeamsManifestV1D27 = TeamsManifestV1D27;
const TeamsManifestV1D28 = __importStar(require("./teams/TeamsManifestV1D28"));
exports.TeamsManifestV1D28 = TeamsManifestV1D28;
...
class AppManifestUtils {
    static getLocalSchemaSuffix(schemaUrl) {
        try {
            const parsedUrl = new URL(schemaUrl);
            if (parsedUrl.hostname === "developer.microsoft.com") {
                const localizedPathMatch = parsedUrl.pathname.match(/^\/[a-z]{2}(?:-[a-z]{2})?(\/json-schemas\/.*)$/i);
     ...
     */
    static getLocalSchemaCandidates(suffix) {
        const candidates = [];
        // 1. Relative to __dirname (works in both source and bundled exe snapshot)
        candidates.push(path_1.default.join(__dirname, "..", suffix));
        // 2. Relative to the running executable (pkg exe layout)
        if (process.execPath) {
            candidates.push(path_1.default.join(path_1.default.dirname(process.execPath), suffix));
        }
        // 3. Relative to cwd
        candidates.push(path_1.default.join(process.cwd(), suffix));
        return candidates;
    }
        ...
        if (suffix) {
            for (const schemaFile of this.getLocalSchemaCandidates(suffix)) {
                if (await fs_extra_1.default.pathExists(schemaFile)) {
                    const raw = await fs_extra_1.default.readFile(schemaFile, "utf8");
