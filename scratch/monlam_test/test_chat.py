...
def get_chat_test_cases():
    ...
    cases.append({"name": "02. Valid user message (Medium)", "url": f"{BASE_URL}/api/v1/ai/chat", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "user", "content": "བཀྲ་ཤིས་བདེ་ལེགས་ ཁྱེད་རང་སྐུ་ཁམས་བཟང་ངམ།"}]}, "expected": [200]})
    cases.append({"name": "03. Valid user message (Long 1,000 chars)", "url": f"{BASE_URL}/api/v1/ai/chat", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "user", "content": "བཀྲ་ཤིས་བདེ་ལེགས། " * 70}]}, "expected": [200]})
    cases.append({"name": "04. Very long input (Bounded to MAX_CHAT_CHARS=5000)", "url": f"{BASE_URL}/api/v1/ai/chat", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "user", "content": "བཀྲ་ཤིས་བདེ་ལེགས། " * 300}]}, "expected": [200]})
    ...
    cases.append({"name": "06. SSE Streaming with Long Prompt", "url": f"{BASE_URL}/api/v1/ai/chat/stream", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "user", "content": "Write a paragraph in Tibetan: " + "བཀྲ་ཤིས་ " * 50}]}, "expected": [200, 404, 405]})
    ...
    cases.append({"name": "09. Special Tibetan Punctuation (༺ ༄༅༎ ༎ ༻)", "url": f"{BASE_URL}/api/v1/ai/chat", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "user", "content": "༺ ༄༅༎ བཀྲ་ཤིས་བདེ་ལེགས་ ༎ ༻"}]}, "expected": [200]})
    cases.append({"name": "10. Tibetan Stacked Characters (ཀྵྲཱིཾཿ)", "url": f"{BASE_URL}/api/v1/ai/chat", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "user", "content": "ཀྵྲཱིཾཿ སངྒྷཿ དྷརྨཿ"}]}, "expected": [200]})

    # 11-20: Script, Emojis, Roles & History
    cases.append({"name": "11. Emojis and Tibetan script mixed", "url": f"{BASE_URL}/api/v1/ai/chat", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "user", "content": "🙏 བཀྲ་ཤིས་བདེ་ལེགས། 🌸 ✨"}]}, "expected": [200]})
    ...
    cases.append({"name": "21. Multilingual English-Tibetan-Chinese", "url": f"{BASE_URL}/api/v1/ai/chat", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "user", "content": "Hello བཀྲ་ཤིས་བདེ་ལེགས་ 你好"}]}, "expected": [200]})
    cases.append({"name": "22. System Prompt Customization", "url": f"{BASE_URL}/api/v1/ai/chat", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "system", "content": "You are a classical Tibetan grammar expert."}, {"role": "user", "content": "Explain ལ་དོན"}]}, "expected": [200]})
    ...
    cases.append({"name": "38. Tibetan Numerals (༡ ༢ ༣ ༤ ༥ ༦ ༧ ༨ ༩ ༠)", "url": f"{BASE_URL}/api/v1/ai/chat", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "user", "content": "༡ ༢ ༣ ༤ ༥ ༦ ༧ ༨ ༩ ༠"}]}, "expected": [200]})
    ...
    cases.append({"name": "46. Tibetan spell correction query", "url": f"{BASE_URL}/api/v1/ai/chat", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "user", "content": "Correct spelling: སློབ་སྦྱང"}]}, "expected": [200]})
    cases.append({"name": "47. Tibetan translation query", "url": f"{BASE_URL}/api/v1/ai/chat", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "user", "content": "Translate to English: བཀྲ་ཤིས་བདེ་ལེགས།"}]}, "expected": [200]})
    cases.append({"name": "48. Tibetan summarization query", "url": f"{BASE_URL}/api/v1/ai/chat", "headers": headers_valid, "payload": {"model_name": "melong", "messages": [{"role": "user", "content": "Summarize this text: " + "བཀྲ་ཤིས་ " * 20}]}, "expected": [200]})
