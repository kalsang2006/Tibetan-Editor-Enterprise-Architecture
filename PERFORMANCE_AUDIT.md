...
1. **TiBERT model loading** (~11-28 s cold start) — dominate startup time
...
```python
# Proposed: tests/performance/test_pipeline_perf.py
@pytest.mark.benchmark
def test_incremental_p99_within_nfr(benchmark):
    result = benchmark(daemon.analyze, large_document)
