...
#: Context key marking whether a fault came from the server's own routing or from
#: a registered handler. It lets the client raise a protocol-level exception
#: (``MethodNotFoundError`` and the like) only for faults the *layer* produced,
#: and always raise ``RemoteError`` -- keeping the handler's code -- for faults a
#: *handler* raised, even when a handler pathologically raises an IPC-coded error.
#: A context value, not a model field, so the wire format is unchanged.
FAULT_ORIGIN_KEY = "$origin"
#: The fault was generated by the server's routing or validation.
FAULT_ORIGIN_PROTOCOL = "protocol"
